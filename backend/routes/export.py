...
def export_company_applications():
    ...
    try:
        ...
        task = export_company_applications_csv.delay(company.id, status=status)
        return jsonify({
            'message': 'Export task started',
            'task_id': task.id,
        }), 202
...
def export_admin_applications():
    ...
    try:
        ...
        task = export_admin_applications_csv.delay(status=status)
        return jsonify({
            'message': 'Export task started',
            'task_id': task.id,
        }), 202
...
def export_offer_letter(placement_id):
    ...
    try:
        ...
        if company and company.offer_template:
            # We wrap it in a proper HTML doc to ensure styling if the template lacks it
            full_template = f"""
            <!DOCTYPE html>
            <html>
            <head>
                <style>
                    ...
                    h1, h2, h3 {{ color: #111; }}
                    .footer {{ margin-top: 40px; font-size: 12px; color: #777; text-align: center; border-top: 1px solid #eee; padding-top: 10px; }}
                </style>
            </head>
            <body>
                {company.offer_template}
                <div class="footer">Generated by Placement Portal App</div>
            </body>
            </html>
            """
            html_content = render_template_string(full_template, **template_context)
        else:
            html_content = render_template('offer_letter.html', **template_context)
